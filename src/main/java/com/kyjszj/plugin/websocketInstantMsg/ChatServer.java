 ...
 * @Package:com.kyjszj.plugin.websocketInstantMsg
 * @Description:TODO
...
public class ChatServer extends WebSocketServer {
	...
	public void onMessage(WebSocket conn, String message) {
		...
		if (null != message && message.contains("fhadmin886")) {
			String toUser = message.substring(
					message.indexOf("fhadmin886") + 10,
					message.indexOf("fhfhadmin888"));
			message = message.substring(0, message.indexOf("fhadmin886"))
					+ "[私信]  "
					+ message.substring(message.indexOf("fhfhadmin888") + 12,
							message.length());
			ChatServerPool.sendMessageToUser(
					ChatServerPool.getWebSocketByUser(toUser), message);// 向所某用户发送消息
	...
	public void userLeave(WebSocket conn) {
		...
		if (b) {
			...
			String joinMsg = "{\"from\":\"[系统]\",\"content\":\"" + user
					+ "下线了\",\"timestamp\":" + new Date().getTime()
					+ ",\"type\":\"message\"}";
